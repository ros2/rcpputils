...
namespace
{
void set_pthread_attr(pthread_attr_t & native_attr, rcpputils::Thread::Attribute const & attr);
...
: handle_(NativeHandleType{}), name_(attr ? attr->get_name() : std::string{})
{
  ...
  if (attr != nullptr) {
    detail::set_pthread_attr(native_attr, *attr);
  }

  NativeHandleType h;
  r = pthread_create(&h, &native_attr, detail::thread_main, func.get());
...
void set_pthread_attr(pthread_attr_t & native_attr, Thread::Attribute const & attr)
{
  int r;

#if defined(__linux__)
  CpuSet affinity = attr.get_affinity();
